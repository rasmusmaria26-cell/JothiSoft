...
import { PDF_GREEN, PDF_RED, PDF_BORDER, SIGN_MAP_TA, NAKSHATRA_MAP_TA, PLANET_ABBR_TA, normalizeSign } from '../shared/jathagam2.constants'
import { Blank } from '../shared/Blank'
import type { JathagamProfile } from '@/types/jathagam'

interface RasiChartPageProps {
...
export function RasiChartPage({ profile, horoscope }: RasiChartPageProps) {
  ...
  const getPlanetsInSign = (sign: string) => {
    ...
    return horoscope.rasi_chart?.[houseKey] || []
  }
  ...
  return (
    <PageWrapper showU={false}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' }}>
        <div style={{ fontSize: '16px', color: PDF_GREEN }}>
          திரு / திருமதி <Blank value={profile.name} width="240px" /> ஜெனன ஜாதகம்
        </div>
      </div>

      <div style={{ fontSize: '16px', color: PDF_GREEN, marginBottom: '24px' }}>
        ஜெனன நட்சத்திரம் <Blank value={`${NAKSHATRA_MAP_TA[moonNakshatra]} - ${moonPada}`} width="200px" />
      </div>
      ...
      <div style={{ border: `1.5px solid ${PDF_GREEN}`, padding: '8px 24px', margin: '12px auto 28px', width: 'fit-content', textAlign: 'center', borderRadius: '4px' }}>
        <span style={{ color: PDF_GREEN, fontWeight: 'bold', fontSize: '18px', letterSpacing: '0.5px' }}>இராசி</span>
      </div>
        ...
        <tbody>
          ...
          <tr>
            ...
            <td colSpan={2} rowSpan={2} style={{ border: `1.5px solid ${PDF_BORDER}`, textAlign: 'center', verticalAlign: 'middle', padding: '20px', backgroundColor: '#fffdf8' }}>
              <div style={{ color: PDF_GREEN, fontWeight: 'bold', fontSize: '20px', letterSpacing: '1px' }}>இராசி சக்கரம்</div>
            </td>
            {renderCell('Kataka')}
          </tr>
          <tr>
